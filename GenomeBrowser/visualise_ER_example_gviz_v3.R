...
load(file = paste0(dataDirectory, "ERs_optimised_cut_off_max_gap_all_tissues_w_annot_df.rda"))

tissue_optimal_cut_off_max_gap_df <- read_delim(paste0(dataDirectory, "exon_delta_details_optimised_maxgap_cutoff.csv"), delim = ",")

gtex_tissue_name_formatting <- read_delim(paste0(dataDirectory, "OMIM_gtex_tissue_name_formatting.csv"), delim = ",")

ensembl_gene_id_to_symbol_df_v92 <- read_delim(paste0(dataDirectory, "ensembl_gene_id_to_symbol_df_v92.csv"), delim = ",")

# Functions -------------------------------------------------------------------------------------------

##### First level #####

source(paste0(dataDirectory, "generate_txDb_from_gtf.R"))
source(paste0(dataDirectory, "constraint_general_functions.R"))
...
get_gtex_split_read_table_mean_cov_n_samples_df <- function(gtex_tissue_name_formatting){
 ...
 gtex_split_read_table_annotated_paths <- 
    list.files(paste0(dataDirectory, "gtex_split_read_table_annotated_rda/"), full.names = T)
...
print(paste0('ook'))  
  ...
  if(get_conserv == T){
    ...
    range_to_plot_gr_conserv$mean_phast_cons_7 <- NULL
  ...
  ##### plot tracks #####
  png("www/SNCA_OMIM_reannot_example.png", res = 600, width = 10, height = 11.69/2, units = "in")
...
source(paste0(dataDirectory, "conservation_general_functions.R"))
source(paste0(dataDirectory, "query_biomart.R"))
...
ensembl_grch38_v92_genes_txdb <- 
  generate_txDb_from_gtf(gtf_gff3_path = paste0(dataDirectory, "Homo_sapiens.GRCh38.92.gtf"), 
                         output_path = paste0(dataDirectory, "ensembl_grch38_v92_txdb.sqlite"),
                         seq_levels_to_keep = c(1:22, "X", "Y", "MT"), genome_build = "hg38")

aceview_hg38_txdb <- 
  generate_txDb_from_gtf(gtf_gff3_path = paste0(dataDirectory, "AceView.ncbi_37.genes_gff.gff"), 
                         output_path = paste0(dataDirectory, "aceview_hg38_txdb.sqlite"),
